...
@router.get("/{jobroleskill_id}", response_model=schemas.Jobroleskill)
...
@router.delete("/{jobroleskill_id}", response_model=List[schemas.Jobroleskill])
def delete_jobroleskill(
    *,
    ...
    if not jobroleskill:
        raise HTTPException(status_code=404, detail="Skill not found")
    remaining_jobroleskill = crud.jobroleskill.remove(db=db, id=jobroleskill_id)
