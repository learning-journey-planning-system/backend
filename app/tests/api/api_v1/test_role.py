...
def test_delete_role(client) -> None:
    data = load_role.base_data[0]
    id = data["id"]
    response = client.delete(f"{load_role.base_url}{id}")
    assert response.status_code == 200
    assert data not in response.json()


#The role with this role id already exists in the system.
...
#Role not found for delete
def test_delete_role_that_does_not_exist(client) -> None:
    response = client.delete(f"{load_role.base_url}999")
    assert response.status_code == 404
